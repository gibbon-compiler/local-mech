...
(* Function signatures. *)
Definition fun_sig : Type := (list laddr * list located_type * located_type)%type.
Definition fun_env : Type := list (fun_var * fun_sig).
...
(*                                                                   *)
(* Judgment: FE; DI; Γ; Σ; C; A; N ⊢ A'; N'; e : τ                 *)
(*   FE, DI are global contexts (fun env, datacon info).             *)
(* ================================================================= *)
...
Inductive has_type :
  ...
  | T_Var :
      forall FE DI G S0 C A N x tc l r,
        In (x, LocTy tc l r) G ->
        In ((l, r), tc) S0 ->
        has_type FE DI G S0 C A N A N (e_val (v_var x)) (LocTy tc l r)
  ...
  | T_ConcreteLoc :
      forall FE DI G S0 C A N r0 i l r tc,
        In ((l, r), tc) S0 ->
  ...
  | T_Let :
      forall FE DI G S0 C A N A' N' A'' N''
             x e1 e2 tc1 l1 r1 tc2 l2 r2,
        has_type FE DI G S0 C A N A' N' e1 (LocTy tc1 l1 r1) ->
        has_type FE DI (extend_tenv G x (LocTy tc1 l1 r1))
  ...
  | T_LRegion :
      forall FE DI G S0 C A N A' N' r e t,
        has_type FE DI G S0 C (extend_alloc A r AP_None) N A' N' e t ->
        has_type FE DI G S0 C A N A' N' (e_letregion r e) t
  ...
  | T_LLStart :
      forall FE DI G S0 C A N A'' N'' l r e tc' l' r',
        In (r, AP_None) A ->
        ~ In (l, r) N'' ->
        (l', r') <> (l, r) ->
        has_type FE DI G S0
  ...
  | T_LLTag :
      forall FE DI G S0 C A N A'' N'' l lprev r e tc'' l'' r'',
  ...
  | T_LLAfter :
      forall FE DI G S0 C A N A'' N'' l l1 r tc_prev e tc' l' r',
        In (r, AP_Loc (l1, r)) A ->
        In ((l1, r), tc_prev) S0 ->
        ~ In (l1, r) N ->
        ~ In (l, r) N'' ->
  ...
  | T_DataCon :
      forall FE DI G S0 C A N (dc : datacon) (l : loc_var) (r : region_var)
  ...
  (* ---- T-App (thesis: \tapp) ----
     ...
     where N' = N − {(l,r)} *)
  | T_App :
      forall FE DI G S0 C A N (f : fun_var)
             (lrs : list (loc_var * region_var)) (vs : list val)
             (tc : tycon) (l : loc_var) (r : region_var)
             (sig_locs : list laddr) (sig_args : list located_type)
             (sig_out : located_type),
        In (f, (sig_locs, sig_args, sig_out)) FE ->
        In (l, r) N ->
        In (r, AP_Loc (l, r)) A ->
        List.length lrs = List.length sig_locs ->
        List.length vs = List.length sig_args ->
...
with pat_has_type :
  ...
  (* ---- T-Pat (thesis: \tpat) ----
     ...
     where Γ' = Γ ∪ {x_i ↦ ft_i@l'_i^r'_i},
           Σ' = Σ ∪ {(l'_i,r'_i) ↦ ft_i} *)
  | T_Pat :
      forall FE DI tc_s G S0 C A N A' N'
        ...
        pat_has_type FE DI tc_s G S0 C A N A' N'
...
with pats_have_type :
  ...
  | T_PatsNil :
      forall FE DI tc_s G S0 C A N t,
        pats_have_type FE DI tc_s G S0 C A N A N t nil

  | T_PatsCons :
      forall FE DI tc_s G S0 C A N A1 N1 A2 N2 t p ps,
        pat_has_type FE DI tc_s G S0 C A N A1 N1 t p ->
        pats_have_type FE DI tc_s G S0 C A1 N1 A2 N2 t ps ->
        pats_have_type FE DI tc_s G S0 C A N A2 N2 t (cons p ps).
...
Inductive program_has_type : fun_env -> datacon_info -> program -> Prop :=
  | T_Program :
      forall FE DI dds fds main A' N' tc l r,
        Forall (fdecl_has_type FE DI) fds ->
        has_type FE DI
                 nil
                 nil
        ...
        program_has_type FE DI (Program dds fds main).
